...
export async function getMyEarnings(req: Request, res: Response) {
  ...
  // 1️⃣ Total commission earned (Lifetime)
  ...
  // 1b. Current Month Earnings (Gap 4)
  const now = new Date();
  const startOfMonth = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));

  const monthlyAgg = await prisma.ledgerEntry.aggregate({
    where: {
      studentId,
      type: "student_commission",
      createdAt: {
  ...
  const currentMonthEarnings = Number(monthlyAgg._sum.amount) || 0;

  // 2️⃣ Total paid leads
  const totalPaidLeads = await prisma.leadsPipeline.count({
    where: {
      studentId,
      status: "paid",
    },
  ...
  // 3️⃣ Lifetime deal value (optional but powerful)
  const dealAgg = await prisma.leadsPipeline.aggregate({
    where: {
      studentId,
      status: "paid",
    },
  ...
  const lifetimeDealValue = Number(dealAgg._sum.dealAmount) || 0;

  // 4️⃣ Potential amount (leads in pipeline not conversion-approved yet)
  const pendingLeads = await prisma.leadsPipeline.findMany({
    where: {
      studentId,
      status: {
        in: [
      ...
      commissionStatus: 'pending' // Only count if commission isn't already approved/added to balance
    },
    select: {
      dealAmount: true,
      commissionRate: true,
  ...
  const potentialEarnings = pendingLeads.reduce((sum, lead) => {
    ...
    // Use snapshot rate, then tier rate, then student rate
    const rate = lead.commissionRate
      ...
      : (lead.student?.partnerTier?.commissionPercentage
        ...
        : Number(lead.student?.commissionPercent || 5));

    return sum + (Number(lead.dealAmount) * rate) / 100;
  }, 0);

  // 5. Payouts (Aggregated directly from DB)
  const withdrawnAgg = await prisma.studentPayout.aggregate({
    where: {
      studentId,
      status: 'completed'
    },
  ...
  const pendingRequestsAgg = await prisma.studentPayout.aggregate({
    where: {
      studentId,
  ...
  const totalRequestsPending = Number(pendingRequestsAgg._sum.amount) || 0;

  // Available Balance = Lifetime Commission - (Completed Payouts + Pending Requests)
  const walletBalance = totalCommissionEarned - (totalWithdrawn + totalRequestsPending);

  res.json({
    total_commission_earned: totalCommissionEarned,
    current_month_earnings: currentMonthEarnings, // Gap 4
    total_paid_leads: totalPaidLeads,
    lifetime_deal_value: lifetimeDealValue,
    pending_amount: potentialEarnings, // Kept name for compatibility, but this is "Potential"
    wallet_balance: walletBalance,     // NEW: Actual available to withdraw
    total_withdrawn: totalWithdrawn,   // NEW: Total paid out
    total_requests_pending: totalRequestsPending, // NEW
    monthly_breakdown: await getMonthlyBreakdown(studentId)
  });
}
...
export async function getMyClients(req: Request, res: Response) {
  try {
    ...
    const clients = await prisma.client.findMany({
      where: { studentId },
      include: {
        leads: true, // Include leads to get status and deal amount
      },
      orderBy: { createdAt: "desc" },
    });

    // Calculate earnings for each client based on their leads
    const student = await prisma.student.findUnique({
      where: { id: studentId },
      include: { partnerTier: true }
    });

    const percent = student?.partnerTier?.commissionPercentage
      ? Number(student.partnerTier.commissionPercentage)
      : (student?.commissionPercent || 5);

    const transformedClients = clients.map(client => {
      const activeLead = client.leads[0]; // Assuming 1 active lead usually

      // Use lead's snapshot rate if available, fallback to student's current tier/percent
      const leadRate = activeLead?.commissionRate
        ? Number(activeLead.commissionRate)
        : (student?.partnerTier?.commissionPercentage
          ? Number(student.partnerTier.commissionPercentage)
          : Number(student?.commissionPercent || 5));

      const dealAmount = activeLead?.dealAmount ? Number(activeLead.dealAmount) : 0;
      const earnings = Math.floor((dealAmount * leadRate) / 100);

      return {
        ...client,
        calculated_earnings: earnings,
        deal_amount: dealAmount, // Explicitly send number
        payment_status: activeLead?.status || 'pending',
        commission_percent: leadRate
      };
    });
...
export async function getRecentActivity(req: Request, res: Response) {
  try {
    ...
    // Fetch recent events:
    // 1. New Leads (Conversions)
    // 2. Commissions (Payments)
    // 3. Payouts (Milestones/Payments)

    // We can fetch from different tables and merge/sort.

    // Recent Commissions (Payments)
    ...
    const newLeads = await prisma.leadsPipeline.findMany({
      where: {
        studentId
      },
      orderBy: { createdAt: 'desc' },
    ...
    commissions.forEach(comm => {
      // Use paidAt from lead if available, otherwise fall back to ledger entry createdAt
      const activityTime = comm.leadsPipeline?.paidAt || comm.createdAt;
      activities.push({
        id: `comm_${comm.id}`,
        type: 'payment',
        title: 'Commission Approved',
        description: `Earned from ${comm.leadsPipeline?.client?.businessName || 'Client'}`,
        time: activityTime,
        amount: comm.amount,
        rawTime: new Date(activityTime).getTime()
      });
    });

    // Map Leads (Conversions)
    newLeads.forEach(lead => {
      activities.push({
        ...
        time: lead.createdAt,
        amount: lead.dealAmount || undefined, // Potential value
    ...
    payouts.forEach(payout => {
      ...
      activities.push({
        ...
        type: payout.status === 'completed' ? 'payment' : 'milestone',
        title,
        description,
        time: payout.createdAt,
        amount: Number(payout.amount),
        rawTime: new Date(payout.createdAt).getTime()
      });
    });
    ...
    // Return top 10
    const topActivities = activities.slice(0, 10).map(a => {
      // Format relative time (simple version, frontend can do better)
      ...
      else if (diffHrs > 0) relativeTime = `${diffHrs} hour${diffHrs > 1 ? 's' : ''} ago`;
...
export async function getNotifications(req: Request, res: Response) {
  try {
    ...
    // Check if user has any notifications, if not seed some for demo
    const count = await prisma.notification.count({ where: { studentId } });
    if (count === 0) {
        ...
        data: [
          {
            studentId,
            type: 'milestone',
            title: "🎯 You're close to next payout!",
            description: "Just ₹250 more to reach the ₹1,000 minimum payout threshold",
          ...
          },
          {
            studentId,
            type: 'conversion',
            title: "🎉 Client converted – take follow-up action",
            description: "ConstructPro LLC signed up! Send a thank you message.",
            amount: 925,
          ...
          },
          {
            studentId,
            type: 'system',
            title: "🔥 Top partners did this today",
            description: "They shared on LinkedIn between 10-11 AM and got 3x more clicks",
    ...
    const formatted = notifications.map(n => {
      ...
      else if (diffHrs > 0) time = `${diffHrs} hour${diffHrs > 1 ? 's' : ''} ago`;
      else time = 'Just now';
